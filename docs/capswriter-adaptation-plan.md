...
| 模型矩阵 | Paraformer、SenseVoice、Fun-ASR-Nano、Qwen3-ASR | sherpa-onnx fast/balanced/fallback，accurate 外部命令实验档 | 增加模型 manifest 和可插拔后端说明，Qwen3/FunASR 作为外部准确率后端 |
...
- 已完成批量模型包发布与导入验收底座：可从当前 full 包一次生成已有非 planned 模型包，产出 zip 大小与 SHA-256 清单，并验证模型包导入到复制的 core 包。
- 已完成 Release 资产打包底座：公开默认生成 core portable zip 与 SHA-256 manifest，full 包和模型包改为显式开关；实际上传 GitHub Release 需要 `gh` 或 API token。
- 已完成常驻 ASR worker：同一档位可复用已加载 recognizer，保留隔离 worker 作为崩溃兜底。
- 已完成用户热词和规则：`hot.txt` 与 `hot-rule.txt` 可由设置页直接打开，确定性应用在 LLM 之前；数据页可用 `词表试算` 检查一句话的规范化、内置词表、热词、规则、ITN 和收尾结果。
- 已完成基础 push-to-talk：默认 `CapsLock` 与鼠标 `X2` 长按录音、松开转写，设置页可改触发键/侧键/吞键和长按阈值；短按 CapsLock 会补发原按键。
- 已完成热键可诊断性增强：设置页和 Doctor 会显示重复、格式不可识别、被占用热键的原因和换键建议，GUI 启动不被热键失败阻断。
...
- 已完成 per-app 输入 profile 编辑入口：内置常见聊天、浏览器、文档和 IDE 应用，设置页可编辑进程/类名/标题匹配、粘贴延迟和标点策略；默认仍不自动 Enter。
- 已完成基础 ITN 和标点策略：中文数字、百分比、金额、日期、时间、范围、单位可确定性转换；聊天类短句可去末尾句号。
- 已完成托盘常驻底座：关闭主窗口隐藏到托盘，托盘菜单可显示主窗、录音、打开目录、诊断和退出。
- 已完成翻译引擎抽象底座：默认 `llm` 路线保持 MiniCPM 体验，新增 `external` 本地命令后端，`nllb`/`bergamot` 作为后续内置适配预留。
- 已完成翻译基准和 external 验收底座：CLI 和设置页可输出中日英短句 CSV，便携包带 mock external translator 验证 stdin/stdout JSON 管道。
- 已完成历史追踪底座：每条历史保存 raw ASR、词表/热词/规则/ITN 阶段、LLM 最终文本和 ASR/清理/LLM/总耗时。
- 已完成历史对比底座：历史详情可显示 raw ASR 到最终文本的字符级 diff，快速判断纠错来源。
- 已完成设置分组底座：设置拆成语音、模型、输入、智能、快捷键、数据；模型页可直接编辑 ASR 模型路径，输入页可编辑 per-app 策略，方便主体/模型分离和移动硬盘测试。
- 已完成诊断导出底座：数据页可一键导出 config、history、词表、日志、doctor 报告和模型 manifest 的 zip，明确排除录音和模型二进制。
- 已完成 ASR 预热底座：常驻 worker 模式下启动空闲会尝试后台预热当前可用 profile，模型页也提供手动预热按钮。
- 已完成模型档位基准入口：模型页每个 ASR profile 和 CLI `--benchmark-asr-profile` 都可直接选择样本目录跑 CSV benchmark，便于对比 fast/balanced/fallback/accurate 的真实延迟和 CER。
- 已完成 ASR 基准样本模板：CLI `--write-asr-benchmark-template` 可在目标机器生成 10 条中文参考 `.txt` 和 README，不覆盖已有录音/文本。
- 已完成 `accurate` 实验后端接口：`asr.accurate_external_command` 通过 JSON stdin/stdout 接入 Qwen3/FunASR 包装脚本或本地服务，发布门禁用 `Mock-External-Asr.ps1` 验证管道；真实大模型评估仍不进入默认核心包。
- 已完成确定性测试后端：`asr.default_engine=mock` 可无模型跑 ASR benchmark，`mock://echo` / `mock://translate` / `mock://fixed/<text>` 可无 MiniCPM 跑纠错/翻译测试，用于发布门禁和后续 UI/state 自动化。
- 已完成历史筛选底座：历史页可按文本/阶段内容、后端、模型和日期过滤，方便从大量记录里定位问题样本。
- 已完成取消状态底座：后端补齐 `Cancelling`，清空/重录会换 session token，旧 ASR/纠错/翻译/error 结果不会覆盖当前 UI。
...
- 新增文件 watcher：热词保存后 1-3 秒内重载；当前已做到下一次转写即时读取，Settings / Data 显示词条数、别名数、规则数和无效规则，Doctor 同步提醒。
- 新增 GUI 热词页：打开文件、添加词条、测试一句、查看命中日志。当前数据页已提供打开文件、统计和一句话试算，独立热词页与命中日志仍是后续增强。
...
- `accurate` profile：实验性接 Qwen3-ASR/Fun-ASR-Nano 路线，当前作为外部本地命令/服务适配，不进入默认主链路。
...
| A02 | `src-tauri/src/lib.rs` | 全局快捷键改为按下/抬起事件驱动，无法注册时不阻塞 GUI | CapsLock/X2 冲突时主窗仍能打开；重复/无效/被占用快捷键会给出换键建议 |
...
| A08 | `src-tauri/src/text.rs` | 新增 `hot.txt` 解析和热重载 | 下一次转写即时读取，Settings / Data 显示热词条数和别名数 |
| A09 | `src-tauri/src/text.rs` | 新增规则替换 | 支持 `查找 = 替换` 和捕获组，Settings / Data 与 Doctor 显示无效正则规则 |
| A10 | `src-tauri/src/itn.rs` | 新增中文 ITN | 数字、范围、日期、金额样例通过 |
| A11 | `src-tauri/src/asr.rs` | 增加 profile manifest 和 checksum | 模型缺失提示精确到文件；模型页每个 profile 可直接跑 ASR benchmark |
| A12 | `src-tauri/src/asr.rs` | recognizer worker 常驻/预热 | 已支持常驻 worker、启动空闲预热和手动预热 |
| A13 | `src-tauri/src/asr.rs` | `accurate` 实验后端接口 | 已可通过 `asr.accurate_external_command` 接入 Qwen3/Fun-ASR 外部本地命令 |
| A14 | `src-tauri/src/doctor.rs` | 新增诊断命令 | 输出环境报告和修复建议 |
| A15 | `src-tauri/src/history.rs` / `src/main.ts` | 历史加入 raw/hot/rule/final/stage timings 和对比视图 | 已保存 raw、词表、热词、规则、ITN、LLM、阶段耗时，可按文本/后端/模型/日期筛选，并在详情里显示 raw-to-final diff |
| A16 | `src-tauri/src/llm.rs` | LLM 角色和翻译分离 | 已将翻译入口迁出 LLM 模块，默认 LLM 翻译保持可用 |
| A17 | `src-tauri/src/translation.rs` | 新增专用 MT 后端抽象 | 已支持 `llm` / `external`；中日英短句可接外部本地 MT，并有翻译 benchmark CSV、per-click translation log、Doctor 最近翻译诊断和 packaged mock external acceptance |
| A18 | `src/main.ts` | 设置拆为语音、模型、词表、输入、翻译、数据 | 已拆成语音、模型、输入、智能、快捷键、数据六组；热词/规则入口位于数据页 |
| A19 | `src/styles.css` | 保持毛玻璃，但给词表/模型/诊断做密集工具布局 | 数据页已加入词表试算，UI smoke 覆盖结果行；没有网页感滚条和文字溢出 |
