...
- 已完成翻译基准底座：CLI 和设置页可输出中日英短句 CSV，用于记录耗时、错误、目标语言提示和说明性输出过滤。
...
| A17 | `src-tauri/src/translation.rs` | 新增专用 MT 后端抽象 | 已支持 `llm` / `external`；中日英短句可接外部本地 MT，并有翻译 benchmark CSV 追踪耗时和异常输出 |
