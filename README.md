...
多台电脑共用同一份模型仓库时，可以用三种方式指定模型根目录：系统环境变量 `VOICE_IME_MODEL_DIR`、设置页“模型根目录”，或在隐藏的 `app` 目录里创建 `MODEL_ROOT.txt`，第一行写模型仓库路径。双击 `启动语音输入.bat` 时会读取 `app\MODEL_ROOT.txt`，托盘菜单“模型目录”、诊断页和模型状态会指向同一个有效目录。
