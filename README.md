...
多台电脑共用同一份模型仓库时，可以用三种方式指定模型根目录：系统环境变量 `VOICE_IME_MODEL_DIR`、设置页“模型根目录”，或在隐藏的 `app` 目录里创建 `MODEL_ROOT.txt`，第一行写模型仓库路径。后端、启动脚本、诊断、托盘菜单“模型目录”和 MiniCPM 启动脚本都会读取 `app\MODEL_ROOT.txt`，并指向同一个有效目录。
